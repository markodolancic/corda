...
* ``FinalityFlow`` is now an inlined flow and no longer initiates a handler flow in the counterparty. This is to fix the
  security problem that this handler flow accepts any transaction it receives without any checks. Existing CorDapps relying
  on this old behaviour will continue to function as previously but new ones will be required to use this new API. See BLAH
  for further details.
