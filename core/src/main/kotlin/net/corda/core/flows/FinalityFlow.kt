 ...
 * the final notarised transaction, and to do that they must be running a counterpart flow which calls [ReceiveFinalityFlow].
 * Sessions with non-participants can also be included, but they must pass in true for [ReceiveFinalityFlow.recordAllStates]
 * if they wish to record the contract states in their vaults.
 *
 ...
 *
 * All parties who are receiving a finalised transaction from a sender flow must subcall this flow.
 *
 * It's typical to have already signed the transaction proposal in the same workflow using [SignTransactionFlow]. If so
 * then the transcation ID can be passed in as an extra check to ensure the finalised transaction is the one that was signed
